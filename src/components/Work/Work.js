...
function Work() {
  ...
  const OFFSET = scrollerStyles.width - window.innerWidth + dragInfo.translation.x
  ...
  useEffect(() => {
    ...
    const expandAnimation = (top, left) => {
      ...
      const OFFSET_X = dragInfo.translation.x - ((window.innerWidth - ITEM_WIDTH) * .5)
      const NEW_X = OFFSET_X + left

      const OFFSET_Y = dragInfo.translation.y - ((window.innerHeight - ITEM_HEIGHT) * .5)
  ...
  return (
    ...
    <div className={`work__inner view__child -wrap-nav ${shouldBeVisible ? 'active': ''}`}>
      ...
      {windowSize.width > 475 && <div 
        ...
        { caseStudies.map(({ uid, tags, data: { header, looping_video, color, svg, copy } }, index) => {
    ...
    <CSSTransition in={showModal} timeout={400} classNames="fade-slide" mountOnEnter unmountOnExit>
      <WorkReelModal setShowModal={setShowModal} video={home.video_reel.url}/>
    </CSSTransition>
    </div>
  )
}
  ...
  return (
    <div>
      ...
      >
        ...
        { data.slice(0, 3).map(({ uid, data: { thumbnail, svg, header } }, index) => {
          const mobileImage =header[0]?.mobileImage
          const joinedCopy = header[0].copy.map(item => item.text).join('')
...
const WorkVideoThumb = ({left, top, dragging, setShowModal, video}) => {
  ...
  return (
    <div className="work__item__video" style={{left, top}} onClick={() => {
      ...
      {video && video.url && <div className="work__item__video--wrapper">
        ...
        <div className="work__list__container">
          <div className="work__list__items">
            {items.map((caseStudy,index) => {
              ...
              const commonTags = filters.tags.filter(filter => csTags.includes(filter))

              return (
                  <Link to={`/work/${caseStudy.uid}`} key={`work-list-item${index}`}>
                <div className={`work__list__item ${commonTags.length === 0 ? 'disable': ''}`}>
...
const WorkReelModal = ({setShowModal, video}) => {
  ...
  const windowSize = useWindowSize()
  const [dimensions, setDimensons] = useState({width: 'unset', height: 'unset'})
...
const WorkThumbnail = withRouter(({ history, uid, featured, thumbnail, video, svg, title, dragging, launchProject, top, left, delay, disable, expandAnimation }) => {
    ...
    >
      <div className="work__link__wrapper">
       <img
          ...
          nopin="nopin"
        />
        {video && <div className="work__link__item--wrapper">
